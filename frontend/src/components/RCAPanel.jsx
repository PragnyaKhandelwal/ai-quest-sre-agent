...
export default function RCAPanel({ incident }) {
  ...
  return (
    <div className="mx-4 mb-4 rounded-lg border border-border border-t-2 border-t-cyan bg-panel px-6 py-4">
      <div className="flex items-center justify-between flex-wrap gap-3 mb-3">
        <div>
          <h2 className="text-sm font-semibold uppercase tracking-wider text-gray-300">
            Post-Mortem / RCA -- {rca.title}
          </h2>
          <span className="text-xs text-gray-500 font-mono-log">{incident.incident_id}</span>
        </div>
        <div className="flex gap-2">
          <button
            onClick={downloadJson}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded border border-cyan/40 text-cyan hover:bg-cyan/10 transition"
          >
            <span>⬇</span> Download JSON
          </button>
          <button
            onClick={downloadPdf}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded border border-success/40 text-success hover:bg-success/10 transition"
          >
